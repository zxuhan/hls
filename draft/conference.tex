...
The four instances span distinct structural profiles. MOD\_A is the
largest and most constraint-rich, exercising every constraint type the
formulation defines. MOD\_B has a shallow DAG and no state activity, so
its difficulty lies in spatial and tool exclusivity. MOD\_C is small
but precedence-dense for its size. MOD\_D is the most precedence-heavy
with negligible state activity, isolating dense precedence from
...
mean-gap delta of $+1.3$ percentage points. On the 9 non-zero pairs a
paired Wilcoxon signed-rank test gives $W = 13.5$, $p = 0.25$, so the
median paired difference is not significant. Interpretation is
...
modules; H1 and H2 can be revisited under non-parametric tests once
...
reference. The verdict is preliminary at $N = 4$; further industrial
instances, finer budget sweeps, and multi-seed CP-SAT replication are
