...
is the dominant economic cost. On the one module for which a manual
schedule was recorded, that schedule spans 31 days, twelve more than
the 19-day optimum found in this study. This paper investigates
algorithms to replace this manual process.
...
\begin{itemize}
  ...
  15\%.\footnote{The 15\% threshold is a pre-registered,
  industrially-motivated design choice, fixed before the multi-instance
  runs.}
\end{itemize}

This study makes four contributions. First, it formalises the ASML
machine upgrade scheduling problem as an RCPSP with five domain-specific
...
industrial instances. Third, it characterises the speed and quality
...
returns a feasible schedule. Fourth, it shows that a critical-path ordering closes most of the
...
by Brucker et al.~\cite{brucker1999rcpsp}, with a comprehensive treatment
...
rule of Vil\'im~\cite{vilim2011fastcumulative} and the conditional
...
most-total-successors rule we adopt as one baseline of Candidate A;
the other, shortest processing time, is a standard duration-based
dispatching rule used as a simple contrast baseline. Metaheuristics such as genetic
...
Hartmann~\cite{kolisch2006experimental} benchmarks many of these on
PSPLIB, where genetic algorithms are among the strongest performers. Hartmann and Briskorn~\cite{hartmann2010survey}
...
available engineers throughout; hours outside the block have no staff
and no work is assigned.
...
instantaneous, assuming no switching cost across machine states.
...
limit is a cumulative constraint~\cite{vilim2011fastcumulative}
...
where CP-SAT terminates within the budget, otherwise the best
feasible solution found at the cap (the incumbent). Where CP-SAT does not prove optimality, DEH's gap
...
every instance; here this is $(w, \gamma) = (0, 0)$
(Table~\ref{tab:deh-grid}). This is an in-sample choice; fixing the configuration before any evaluation runs
...
On MOD\_B the CP-SAT entry is the incumbent at the 600-second cap,
not a proved optimum.}
...
$m^{*}$ raises the cross-instance mean gap from 6.1\% to 12.1\%,
...
\begin{table}[ht]
\caption{Cross-instance hypothesis summary. The full DEH
$(w, \gamma)$ grid is in Table~\ref{tab:deh-grid}.}
\label{tab:h1summary}
\centering
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{4pt}
...
\begin{table}[t]
\caption{DEH mean optimality gap (\%) over the four instances across
weight $w$ and CST setting $\gamma$. The minimum is the recommended
$(w, \gamma) = (0, 0)$; at $\gamma = 0$ the mean gap is non-decreasing
in $w$.}
\label{tab:deh-grid}
\centering
\renewcommand{\arraystretch}{1.15}
...
Across the full $(w, \gamma)$ grid (Table~\ref{tab:deh-grid}), the
lowest-mean-gap DEH configuration is $w = 0, \gamma = 0$ at 6.1\%,
below MTS at 9.7\% and SPT at 31.0\%: a 37\% relative reduction versus
the best classical priority rule. DEH at $(w=0, \gamma=0)$ also never
degrades against MTS or SPT on any individual instance, which H1 does
not by itself require.
...
\begin{table}[t]
\caption{CST ablation: makespan change in days from enabling CST
($\gamma = 0.5$ minus $\gamma = 0$) at each instance and weight.
Negative favours CST, positive degrades, zero ties. The split is 3
wins, 11 ties, and 6 losses; the only gains are at intermediate $w$ on
MOD\_D.}
\label{tab:cst-delta}
\centering
\renewcommand{\arraystretch}{1.15}
...
\toprule
Inst.\ & $w{=}0$ & $w{=}0.25$ & $w{=}0.5$ & $w{=}0.75$ & $w{=}1.0$ \\
\midrule
MOD\_A & $+1$ & $+3$ & $-1$ & $0$ & $0$ \\
MOD\_B & $0$  & $+1$ & $+1$ & $+1$ & $+1$ \\
MOD\_C & $0$  & $0$  & $0$  & $0$  & $0$ \\
...
Table~\ref{tab:cst-delta} shows the per-cell makespan effect of CST:
equal-or-better in 14 of 20 pairs but losing twice as often as it
wins. At the recommended $w = 0$ it degrades only MOD\_A, by one day
(a $+1.3$ percentage-point mean-gap delta), and ties elsewhere.
...
comparison, appropriate for the H1 and H2 verdicts but not for the
sub-second regime the research question targets
(Section~\ref{sec:rqs}). To probe that regime, the budget sweep varies
CP-SAT's wall-clock budget from 0.1 to 600 seconds
...
MOD\_B, which combines a heavy workplace zone constraint (31 zones across 90
...
sets $w = 0$. At $\gamma = 0$ the mean gap is non-decreasing in $w$
(Table~\ref{tab:deh-grid}), so weighting state continuity is a worse
ordering criterion than critical-path priority on this set. CST is
similar (Table~\ref{tab:cst-delta}): at the recommended $w = 0$ it
costs one day on MOD\_A, and its only gains are at intermediate $w$ on
MOD\_D, the deepest, densest instance. CST is therefore retained as an
optional per-instance signal rather than part of the recommended
configuration: it may be worth enabling on structurally similar
modules as the instance set grows, but it is not load-bearing for the
headline comparison.

\subsection{Practical Implications}

The speed gap is decisive for interactive scenario iteration. CP-SAT
hits the 600-second cap on MOD\_B, so a hundred-scenario sweep
...
Section~\ref{sec:per-instance-results}). Its profile (heavy workplace zone
constraint, an exclusive tool, minimal state activity) motivates
...
sweep shows this: below one second CP-SAT returns no
...
when the objective rewards them. Makespan alone does not help, so the
critical path dominates, while an objective that adds machine state
reconfiguration cost would change that calculus.
...
\bibitem{vilim2011fastcumulative}
P.~Vil\'im,
...
Programming (CPAIOR)}, 2011, pp.~230--245.
