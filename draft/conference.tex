...
the working hours of each day, walking days in input order until a
...
task fits inside the working hours of a single day. The horizon $H$ is
the full length of the $|D|$ scheduled days in hours ($H = 24 |D|$);
the grid spans the full 24 hours of each day,
with non-working hours appearing as fixed zero-capacity windows so the
cumulative constraint can encode them as $C(t)=0$ runs (see the
deficit-interval construction below).

Let $C(t)$ denote the instantaneous staff capacity at time $t$ (zero
if $t$ lies outside working hours) and $C^{*} = \max_{t \in [0,H]} C(t)$
the peak staff capacity. The staffing constraint is a cumulative
...
module with an exclusive tool (MOD\_B), and a deep dense
...
depth. On MOD\_B, which combines an exclusive tool
...
structural profile (heavy zone footprint, an exclusive
...
evaluated under a fixed working-day configuration (14 working hours per
day, six staff, 30-day horizon); a horizon-and-staffing sweep is a
